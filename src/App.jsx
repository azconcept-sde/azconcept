...
const MONTHLY = [
  {mes:"Ene",rev:42000},{mes:"Feb",rev:58000},{mes:"Mar",rev:71000},
  {mes:"Abr",rev:54000},{mes:"May",rev:89000},{mes:"Jun",rev:103000},
];
...
export default function App() {
  ...
  // ── Metrics ───────────────────────────────────────────────────────────────
  const totalRev  = products.reduce((s,p)=>s+(p.price||0)*(p.sales||0),0);
  const totalCost = products.reduce((s,p)=>s+(p.cost||0) *(p.sales||0),0);
  const profit    = totalRev-totalCost;
  const topViewed = [...products].sort((a,b)=>(b.views||0)-(a.views||0)).slice(0,5);
  const topSold   = [...products].sort((a,b)=>(b.sales||0)-(a.sales||0)).slice(0,5);
    ...
    <div style={{fontFamily:"'Nunito Sans',sans-serif",background:B.bg,minHeight:"100vh",color:B.ink}}>
      ...
      {view==="store"&&(
        <>
          ...
          <div style={{background:`linear-gradient(90deg,${B.orange},${B.pink})`,padding:"12px 28px",display:"flex",justifyContent:"center",gap:40,flexWrap:"wrap"}}>
            {["✦ Todo 100% personalizado","✦ Envíos a toda Argentina","✦ Pagá con Mercado Pago"].map(t=>(
          ...
          <section style={{padding:"18px 28px 64px",display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))",gap:22,maxWidth:1140,margin:"0 auto"}}>
            ...
            {filtered.map(p=>(
              <div key={p.id} className="pc" onClick={()=>{setSelected(p);setCustomText("");}}
      ...
      {checkout&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div style={{background:B.white,borderRadius:18,maxWidth:440,width:"100%",padding:34,maxHeight:"90vh",overflowY:"auto"}}>
            {checkout==="form"&&(
              <>
                ...
                <div style={{display:"flex",gap:12,marginTop:28}}>
                  ...
                  <button onClick={()=>setCheckout("mp")} style={{flex:2,...btnDark,padding:12,borderRadius:7}}>Continuar →</button>
                </div>
              </>
            )}
            {checkout==="mp"&&(
                  ...
                  <div>
                    <div style={{background:"#FFFBEB",borderRadius:10,padding:14,marginBottom:20,border:"1px solid #FDE68A",display:"flex",gap:10,alignItems:"flex-start"}}>
                      ...
                      <p style={{fontSize:13,color:"#92400E",lineHeight:1.6}}>
                        <strong>Modo simulación:</strong> configurá Mercado Pago en el Panel → Configuración.
                      </p>
                    </div>
                    <button onClick={()=>{setCheckout("ok");setCart([]);}} style={{width:"100%",background:"#009EE3",color:"white",border:"none",borderRadius:8,padding:15,fontWeight:800,fontSize:15,cursor:"pointer",marginBottom:12,fontFamily:"'Nunito Sans',sans-serif"}}>
                      💳 Simular pago con Mercado Pago
                    </button>
                  </div>
                )}
                <button onClick={()=>setCheckout("form")} style={{width:"100%",background:"none",border:"none",color:B.muted,cursor:"pointer",fontSize:13,fontFamily:"'Nunito Sans',sans-serif"}}>← Volver</button>
              </>
            )}
            {checkout==="ok"&&(
              <div style={{textAlign:"center",padding:"10px 0"}}>
                ...
                <h2 style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontWeight:700,fontSize:26,marginBottom:10}}>¡Pedido recibido! ✦</h2>
                <p style={{color:B.muted,fontWeight:400,marginBottom:10,lineHeight:1.7}}>Te contactamos por WhatsApp al <strong style={{color:B.ink}}>+54 9 3855 02-7443</strong> para coordinar tu diseño.</p>
                <p style={{color:B.muted,fontWeight:400,marginBottom:28,fontSize:13}}>¡Gracias por elegir <strong style={{color:B.orange}}>AZ Concept</strong>!</p>
                <a href={WA_LINK} target="_blank" rel="noreferrer" style={{display:"inline-block",background:"#25D366",color:"white",borderRadius:8,padding:"12px 24px",fontWeight:700,fontSize:14,textDecoration:"none",marginBottom:12,fontFamily:"'Nunito Sans',sans-serif"}}>💬 Escribirnos ahora</a><br/>
                <button onClick={()=>setCheckout(null)} style={{background:"none",border:`1.5px solid ${B.line}`,borderRadius:8,padding:"10px 22px",fontWeight:600,fontSize:13,cursor:"pointer",marginTop:8,fontFamily:"'Nunito Sans',sans-serif"}}>Seguir explorando</button>
              </div>
            )}
          </div>
        </div>
          ...
          {panelTab==="metricas"&&(
            <>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))",gap:16,marginBottom:24}}>
                {[
                  {label:"Ingresos totales", value:fmt(totalRev), icon:<DollarSign size={20}/>,color:B.ink},
                  {label:"Ganancia neta",    value:fmt(profit),   icon:<TrendingUp size={20}/>,color:B.verde},
                  {label:"Unidades vendidas",value:products.reduce((s,p)=>s+(p.sales||0),0), icon:<Package size={20}/>,color:B.orange},
                  {label:"Consultas totales",value:products.reduce((s,p)=>s+(p.views||0),0), icon:<Eye size={20}/>,color:B.pink},
                ].map((kpi,i)=>(
                  <div key={i} style={card}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div>
                        ...
                        <p style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontSize:24,fontWeight:700,color:kpi.color}}>{kpi.value}</p>
                      </div>
              ...
              <div style={{...card,marginBottom:24}}>
                <h3 style={{fontFamily:"'Bricolage Grotesque',sans-serif",fontWeight:700,fontSize:16,marginBottom:20}}>Ingresos mensuales (ARS)</h3>
                <ResponsiveContainer width="100%" height={230}>
                  <BarChart data={MONTHLY} barSize={38}>
 ...
 para hacer un salto de línea</p>
            ...
            <div style={{maxWidth:560}}>
              <div style={card}>
                ...
                <div style={{background:B.bg,borderRadius:8,padding:14,marginTop:20,border:`1px solid ${B.line}`}}>
                  <p style={{fontSize:12,color:B.muted,lineHeight:1.7}}>
                    <strong>Cómo obtenerla:</strong><br/>
                    1. Entrá a <strong>mercadopago.com.ar</strong><br/>
                    2. Tu negocio → Credenciales<br/>
                    3. Copiá la <strong>Public Key</strong> de producción<br/>
